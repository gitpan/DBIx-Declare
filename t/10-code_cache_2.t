...
SKIP: {
	{
		...
		diag($@) if $@;
		skip ": DBD::ODBC not available", 28 unless $drh;
	}

	{
		...
		unless ($dbh) {
			diag($DBI::errstr);
			skip ": local MSSQL server connection failed", 28 ;
		}
	}
...
skip ": the cache directory $cache_dir doesn't exist. please run the 09-code_cache_1.t first", 28
	if (! -d $cache_dir);

skip ": please run the 09-code_cache_1.t first", 28
	if time() - (stat("$cache_dir/placeholder"))[9] > 100;


diag( "cache directory=$cache_dir");

sleep(1); # to ensure the cached code files are at least one second old
my $start_time = time();
...
ok( scalar(@files) >= 10, "There should be at least 10 files in the cache directory");
...
ok( scalar(@files) >= 10, "... not older than 2 minutes");

@files = grep {(stat($_))[9] < $start_time} @files;

#use Data::Dumper;
#print Dumper(\@files);

ok( scalar(@files) >= 10, "... but older than the execution of this script");
