...
SKIP: {
	{
		...
		diag($@) if $@;
		skip ": DBD::ODBC not available", 29 unless $drh;
	}

	{
		...
		unless ($dbh) {
			diag($DBI::errstr);
			skip ": local MSSQL server connection failed", 29 ;
		}
	}

my $cache_file;
BEGIN {
	$cache_file = cwd() . '/code_cache_code.dat';
	($cache_file) = ($cache_file =~ /(.*)/);
}
diag( "cache file=$cache_file");

{
	open my $OUT, '>', $cache_file or skip ": could not create an empty code cache file: $^E ", 29;
	...
	close $OUT or skip ": could not save the code cache file: $^E", 29;
}
...
is( $hits, 0, "The cache was empty, there should be no hits");
is( $misses, 10, "There should be 10 cache misses logged");
is( $sets, 10, "and 10 times some code was stored");
